 ...
 *
 * Two rule classes are shown in separate tabs:
 *   Production (FP-XXXX) — authored manually from threat intel / hypotheses.
 *   Synthetic  (SYN-XXXX) — generated by generate_logs.py; read-only in the UI.
 *
 * Synthetic rules can be tested (they're valid KQL) but cannot be edited or
 * archived — they're managed alongside the log generator, not through the UI.
 */
...
type TTab = "all" | "production" | "synthetic";
...
export default function Detections() {
  ...
  const productionCount = rules.filter((r) => !r.synthetic).length;
  const syntheticCount = rules.filter((r) => r.synthetic).length;

  const visibleRules = rules.filter((r) => {
    if (tab === "production") return !r.synthetic;
    if (tab === "synthetic") return r.synthetic;
    return true;
  });

  const TAB_STYLE = (t: TTab) =>
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-1 mb-4">
        <button className={TAB_STYLE("all")} onClick={() => setTab("all")}>
          All ({rules.length})
        </button>
        <button className={TAB_STYLE("production")} onClick={() => setTab("production")}>
          Production ({productionCount})
        </button>
        <button className={TAB_STYLE("synthetic")} onClick={() => setTab("synthetic")}>
          Synthetic ({syntheticCount})
        </button>
        {tab === "synthetic" && (
          <span className="ml-3 text-xs text-muted-foreground">
            Synthetic rules are read-only — managed by generate_logs.py
          </span>
        )}
      </div>
          ...
          <tbody className="divide-y divide-border">
            {visibleRules.map((rule) => (
              <tr key={rule.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="py-3 pr-4">
                  <button
                    onClick={() => !rule.synthetic && handleToggle(rule)}
                    disabled={rule.synthetic}
                    title={rule.synthetic ? "Synthetic rules are managed by generate_logs.py" : undefined}
                    className={`w-10 h-5 rounded-full transition-colors ${
                      rule.enabled ? "bg-primary" : "bg-muted"
                    } ${rule.synthetic ? "opacity-40 cursor-not-allowed" : ""}`}
                  >
                ...
                <td className="py-3">
                  <div className="flex gap-2">
                    ...
                    {rule.synthetic ? (
                      <span
                        className="text-xs text-muted-foreground/40 cursor-not-allowed"
                        title="Synthetic rules are read-only"
                      >
                        Edit
                      </span>
                    ) : (
